 ...
 * @param {boolean} detail  full artwork instead of the compact mark. Only for
 *   the splash, where the mark renders near 70px; below roughly 48px the shelf
 *   contents and clock hands turn to mush, which is why the compact mark exists.
 */
export default function Logo({
  ...
  return (
    <span className={`logo logo-${tone} ${className}`.trim()}>
      {detail ? <LogoMarkDetailed /> : <LogoMark />}
...
/**
 * The full artwork, mirroring src/assets/logo-mark.svg -- the same file
 * scripts/generate-icons.mjs rasterises for the app icons, so the splash and
 * the installed icon show the same thing.
 *
 * Only used above roughly 48px. Everything below that gets LogoMark.
 */
function LogoMarkDetailed() {
  return (
    <svg className="logo-mark" viewBox="0 0 64 64" aria-hidden="true" focusable="false">
      <path
        d="M46.6 12.6a25 25 0 1 1-11-5.9"
      ...
      <path d="M23 36.4h5" stroke="var(--logo-produce)" strokeWidth="2.2" strokeLinecap="round" />
